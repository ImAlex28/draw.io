...
CREATE OR REPLACE TABLE LIBRO_TAB OF LIBRO_OBJ(
    ISBN PRIMARY KEY,
    TITULO NOT NULL,
    AUTOR NOT NULL
...
CREATE OR REPLACE TABLE COMPRA_TAB OF COMPRA_OBJ(
    ID PRIMARY KEY,
    PUNTOS>=0
)NESTED TABLE LINEA_COMPRA STORE AS LINEASCOMPRA_NTAB;

CREATE OR REPLACE TABLE ALQUILER_TAB OF ALQUILER_OBJ(
    ID PRIMARY KEY
)NESTED TABLE LINEA_ALQUILER STORE AS LINEASALQUILER_NTAB;

CREATE OR REPLACE TABLE CLIENTE_TAB OF CLIENTE_OBJ(
    DNI PRIMARY KEY,
    PUNTUACION >= 0,
    CHECK(TIPOCUENTA IN ('Basica', 'Premium'))
)NESTED TABLE COMPRAS STORE AS COMPRAS_NTAB, NESTED TABLE ALQUILERES STORE AS ALQUILERES_NTAB;

CREATE OR REPLACE TABLE ADMINISTRADOR_TAB OF ADMINISTRADOR_OBJ(
    DNI PRIMARY KEY
)NESTED TABLE COMPRAS STORE AS COMPRAS_NTAB, NESTED TABLE ALQUILERES STORE AS ALQUILERES_NTAB;

CREATE OR REPLACE TABLE PEDIDOPROVEEDOR_TAB OF PEDIDOPROVEEDOR_OBJ(
    ID PRIMARY KEY,
    CHECK(ESTADO IN ('Pendiente', 'Recibido','Cancelado'))
)NESTED TABLE LINEAS_PEDIDO STORE AS LINEASPEDIDOPROVEEDOR_NTAB;

CREATE OR REPLACE TABLE PROVEEDOR_TAB OF PROVEEDOR_OBJ(
    ID PRIMARY KEY,
    NOMBRE NOT NULL,
    TELEFONO NOT NULL
);

CREATE OR REPLACE TABLE FACTURA_TAB OF FACTURA_OBJ(
    ID PRIMARY KEY
)NESTED TABLE COMPRAS STORE AS COMPRAS_NTAB, NESTED TABLE ALQUILERES STORE AS ALQUILERES_NTAB;

CREATE OR REPLACE TABLE TARJETA_TAB OF TARJETA_OBJ(
    NUMERO PRIMARY KEY,
    CVV NOT NULL,
    FECHACADUCIDAD NOT NULL
);

CREATE OR REPLACE TABLE DIRECCION_TAB OF DIRECCION_OBJ(
    TIPODEVIA NOT NULL,
    NOMBRE NOT NULL,
    LOCALIDAD NOT NULL
);